...
public function generateHttpClient (json jsonPayload,http:HttpClient httpClient) (json) {
    ...
    string githubToken = config:getGlobalValue("GITHUB_TOKEN");
    http:OutRequest httpOutRequest = {};
    http:InResponse httpInResponse = {};
    httpOutRequest.addHeader("Authorization","Bearer " + githubToken);
    ...
    if(httpConnectorError != null) {
        log:printError("Error in post request : " + httpConnectorError.message);
        logFile:logError("Error in post request : " + httpConnectorError.message,runtime:getCallStack()[1].packageName);
    }
